...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_DELETE(self):
        ...
        if path == "/api/udplog":
            ...
            # Activity-log Clear bug fix: clear the server-side log so the UI
            # "Clear" actually sticks (otherwise the next poll re-fetches the
            # retained entries and they reappear).
            activity_log.clear()
            self._send_json({"ok": True})
...
async function clearLog() {
    // Activity-log Clear bug fix: clear the server-side log first, else the next
    // poll re-fetches the retained entries (resetting lastLogTs alone pulls the
    // whole log back).
